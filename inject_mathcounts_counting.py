...
from mathcounts_curriculum import all_units, build_master
...
def dry_check():
    ...
    for title, _d, content, questions in units:
        ...
        for q in questions:
            assert q["correct_answer"] in q["options"], title
            if bad.search(q["explanation"]):
...
def main():
    ...
    try:
        ...
        for row in verify(cursor, [u["id"] for u in unit_results] + [master_id]):
            ...
            match = "OK" if (ctype == "master" and q == 0) or (q == p and q == EXPECTED_Q) or (ctype == "master") else "MISMATCH"
            if ctype == "master":
                match = "OK" if q == 0 else "MISMATCH"
            elif q != p or q != EXPECTED_Q:
                match = "MISMATCH"
                ok = False
            print(f"  verify id={row['id']} type={ctype} q={q} placeholders={p} → {match}")
            if match != "OK":
                ok = False
        linked = cursor.execute("SELECT COUNT(*) AS n FROM course_units WHERE parent_course_id=%s", (master_id,))
        # fetch
