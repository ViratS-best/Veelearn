"""Shared helpers for Grade 8 math: pre-algebra, diagrams, quizzes."""
...
PHET = {
    ...
    "eq": (
        ...
        "Keep both sides equal while you peel operations off x.",
    ),
    "lines": (
        "Graphing Lines",
        "https://phet.colorado.edu/sims/html/graphing-lines/latest/graphing-lines_all.html",
        "Drag slope and intercept. Watch the line move.",
    ),
    "slope": (
        "Graphing Slope-Intercept",
        "https://phet.colorado.edu/sims/html/graphing-slope-intercept/latest/graphing-slope-intercept_all.html",
        "y = mx + b. Change m and b and read the graph.",
    ),
    "func": (
        "Function Builder",
        "https://phet.colorado.edu/sims/html/function-builder/latest/function-builder_all.html",
        "Drop a number in. See what rule comes out.",
    ),
    "curve": (
        "Curve Fitting",
        "https://phet.colorado.edu/sims/html/curve-fitting/latest/curve-fitting_all.html",
        "Place points. A line of best fit tells the trend.",
    ),
}
...
def slope_line(m=0.5, b=2, title="Slope and intercept", caption=""):
    w, h = 300, 280
    ox, oy = 40, 230
    scale = 40
    x0, x1 = 0, 6
    y0, y1 = b, m * x1 + b
    px0, py0 = ox + x0 * scale, oy - y0 * scale
    px1, py1 = ox + x1 * scale, oy - y1 * scale
    # rise/run from (2, 3) if m=1/2, b=2: (2,3) to (4,4)
    rx0, ry0 = ox + 2 * scale, oy - (m * 2 + b) * scale
    rx1, ry1 = ox + 4 * scale, oy - (m * 4 + b) * scale
    svg = f"""
<svg viewBox="0 0 {w} {h}" width="{w}" role="img" aria-label="{html.escape(title)}">
  <line x1="{ox}" y1="{oy}" x2="280" y2="{oy}" stroke="#0f172a" stroke-width="2"/>
  <line x1="{ox}" y1="{oy}" x2="{ox}" y2="16" stroke="#0f172a" stroke-width="2"/>
  <text x="282" y="{oy + 4}" font-size="13" font-weight="700">x</text>
  <text x="{ox + 6}" y="16" font-size="13" font-weight="700">y</text>
  <line x1="{px0}" y1="{py0}" x2="{px1}" y2="{py1}" stroke="#4f46e5" stroke-width="3"/>
  <circle cx="{ox}" cy="{oy - b * scale}" r="5" fill="#dc2626"/>
  <text x="{ox + 10}" y="{oy - b * scale - 8}" font-size="12" fill="#b91c1c">b = {b:g}</text>
...
"""
    return figure(
        title,
        svg,
        caption or f"y = {m:g}x + {b:g}. Slope m is rise/run. The line crosses the y-axis at (0, {b:g}).",
    )


def pythag_triangle(a=3, b=4, c=5, title="A right triangle", caption=""):
    svg = f"""
<svg viewBox="0 0 260 200" width="260" role="img" aria-label="right triangle {a}-{b}-{c}">
  <polygon points="40,160 200,160 40,40" fill="#fde68a" stroke="#92400e" stroke-width="3"/>
  <rect x="40" y="140" width="20" height="20" fill="none" stroke="#0f172a" stroke-width="2"/>
  <text x="120" y="182" text-anchor="middle" font-size="14">a = {a}</text>
  <text x="14" y="110" font-size="14">b = {b}</text>
  <text x="130" y="90" font-size="14" fill="#b91c1c">c = {c}</text>
</svg>
"""
    return figure(
        title,
        svg,
        caption or f"a² + b² = c². {a}² + {b}² = {a * a} + {b * b} = {c * c} = {c}². The hypotenuse is opposite the right angle.",
    )
...
def sci_shift(title="Scientific notation", caption=""):
    svg = """
<svg viewBox="0 0 360 90" width="360" role="img">
  <text x="20" y="50" font-size="22" font-weight="700">3.2 × 10⁴</text>
  <text x="200" y="30" font-size="14" fill="#1d4ed8">move 4 places right</text>
  <text x="200" y="62" font-size="22" font-weight="700">32,000</text>
</svg>
"""
    return figure(
        title,
        svg,
        caption or "A positive exponent on 10 makes a large number. 3.2 × 10⁴ = 32,000. A negative exponent makes a small number: 3.2 × 10⁻² = 0.032.",
    )


def cylinder_figure(title="A cylinder", caption=""):
    svg = """
<svg viewBox="0 0 220 180" width="220" role="img">
  <ellipse cx="110" cy="40" rx="50" ry="18" fill="#c7d2fe" stroke="#312e81" stroke-width="2"/>
  <path d="M60,40 L60,140 A50,18 0 0 0 160,140 L160,40" fill="#e0e7ff" stroke="#312e81" stroke-width="2"/>
  <ellipse cx="110" cy="140" rx="50" ry="18" fill="#a5b4fc" stroke="#312e81" stroke-width="2"/>
  <text x="170" y="95" font-size="13">h</text>
  <text x="110" y="28" text-anchor="middle" font-size="13">r</text>
</svg>
"""
    return figure(
        title,
        svg,
        caption or "Volume of a cylinder is B × h, and the base is a circle, so V = πr²h. Use 3.14 for π unless the problem says otherwise.",
    )
...
def end_practice() -> str:
    return (
        ...
        "<p>Start easy. Then they get a little trickier. Sketch a graph, a right triangle, or a system if you need to.</p>"
        "<ul>"
