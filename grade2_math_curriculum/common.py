"""Shared helpers for Grade 2 math: simple English, embeds, quizzes."""
...
PHET = {
    ...
    "arith": (
        ...
        "Try addition and subtraction. Watch how the numbers change.",
    ),
    "area": (
        "Area Builder",
        "https://phet.colorado.edu/sims/html/area-builder/latest/area-builder_all.html",
        "Cover a shape with same-size squares. Count rows and columns.",
    ),
    "frac_intro": (
        "Fractions: Intro",
    ...
    "build_frac": (
        ...
        "Build halves, thirds, and fourths with equal pieces.",
    ),
}
...
def end_practice() -> str:
    return (
        ...
        "<p>Start easy. Then they get a little trickier. Take your time. Draw place-value blocks if you need to.</p>"
        "<ul>"
...
def unit_shell(title: str, roadmap_items, body_html: str, final_slots_html: str) -> str:
    return f"""
<h1>{title}</h1>
<p>This is <strong>second grade math</strong>. We use short words. We work with bigger numbers — up to 1,000.</p>
...
def near_int(correct, count=3):
    c = int(correct)
    pool = []
    for delta in (1, -1, 2, -2, 10, -10, 100, -100, 5, 20, -20, 50):
        v = c + delta
        if v != c and v >= 0:
            pool.append(str(v))
    unique = []
