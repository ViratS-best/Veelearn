"""Shared helpers for Grade 4 math: simple English, embeds, quizzes."""
...
PHET = {
    ...
    "area_dec": (
        ...
        "Tenths and hundredths can fill a grid. That is how decimals work.",
    ),
    "frac_eq": (
        "Fractions: Equality",
...
def protractor_box(kid_line="Drag the protractor onto the vertex. Line up one ray with 0. Read the other ray.", angle=None, show=False):
    parts = []
    if angle is not None:
        parts.append(f"angle={int(angle)}")
    if show:
        parts.append("show=1")
        parts.append("practice=0")
    qs = ("?" + "&".join(parts)) if parts else ""
    url = f"protractor-simulator.html{qs}"
    return (
        f'<div class="phet-sim-wrapper" style="border:1px solid #ddd;border-radius:12px;'
        f'overflow:hidden;background:#fff;margin:16px 0;">'
        f'<div style="background:#eef2ff;padding:12px 14px;font-weight:800;">🎮 Play: Protractor Lab</div>'
        f"<p style=\"padding:0 14px;\">{kid_line}</p>"
        f'<div style="position:relative;padding-bottom:78%;height:0;overflow:hidden;min-height:420px;">'
        f'<iframe src="{url}" title="Protractor Lab" style="position:absolute;top:0;left:0;width:100%;'
        f'height:100%;border:0;" allowfullscreen loading="lazy"></iframe></div>'
        f'<p style="padding:8px 14px;"><a href="{url}" target="_blank" rel="noopener">Open Protractor Lab in a new tab</a></p>'
        f"</div>"
    )
...
def end_practice() -> str:
    return (
        ...
        "<p>Start easy. Then they get a little trickier. Take your time. Use place-value charts, area models, or the protractor lab if you need to.</p>"
        "<ul>"
...
def unit_shell(title: str, roadmap_items, body_html: str, final_slots_html: str) -> str:
    return f"""
<h1>{title}</h1>
<p>This is <strong>fourth grade math</strong>. We use short words. We work with numbers to one million, multiply and divide larger numbers, add fractions, read decimals, and measure angles.</p>
...
def near_int(correct, count=3):
    c = int(correct)
    pool = []
    for delta in (1, -1, 2, -2, 10, -10, 100, -100, 5, 20, -20, 50, 1000, -1000, 10000, -10000, 100000):
        v = c + delta
        if v != c and v >= 0:
            pool.append(str(v))
    unique = []
