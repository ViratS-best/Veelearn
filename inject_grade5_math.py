#!/usr/bin/env python3
"""Inject Fifth Grade Math master + 8 units into Aiven.
...
from grade5_math_curriculum import all_units, build_master

GRADE = 5
MASTER_TITLE = "Fifth Grade Math"
RNG_SEED = 20260817
EXPECTED_Q = 80
...
def upsert_master(cursor, creator_id):
    desc = (
        "Fifth-grade math master course: decimals to thousandths, multi-digit operations, "
        "add and subtract fractions with unlike denominators, multiply fractions, divide "
        "unit fractions, volume of rectangular prisms, the coordinate plane, and introductory "
        "algebra (expressions, order of operations, and patterns). "
        "Short words, quizzes after every idea, and 50 practice problems per unit."
    )
    ...
    if row:
        ...
        cursor.execute(
            """
            UPDATE courses
            SET description=%s, content=%s, status='approved', course_type='master',
                gamification_json=%s
            WHERE id=%s
            """,
            (desc, content, GAMIFICATION, master_id),
        )
        action = "updated"
    else:
        cursor.execute(
            ...
            INSERT INTO courses
            (title, description, content, creator_id, status, grade_level, course_type, gamification_json)
            VALUES (%s, %s, %s, %s, 'approved', %s, 'master', %s)
            """,
            (MASTER_TITLE, desc, content, creator_id, GRADE, GAMIFICATION),
        )
        cursor.execute("SELECT LAST_INSERT_ID() AS id")
